Given('um usuario da aplicacao') do
  ...
  @test_client.login('api/v1/users/sign_in', login_params)
...
Then('o login não é realizado') do
  expect(@test_client.headers['HTTP_USER_TOKEN'].nil?).to be_truthy
  expect(@user.token_match?(@test_client.headers['HTTP_USER_TOKEN'])).to be false
end